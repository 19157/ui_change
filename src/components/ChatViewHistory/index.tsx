...
import SpinLoadingFunction, {
  SpinLoadingActionType,
} from "@/components/SpinLoadingFunction";
...
const ChatViewHistory: GenieType.FC<Props> = (props) => {
  const { inputInfo: inputInfoProp, product, newSessionId } = props;
  ...
  const sendDataMessage = (inputInfo: any) => {
    const params = {
      ...
      searchEnabled: inputInfo.searchEnabled ? 1 : 0,
    };
    const currentChat = {
      query: inputInfo.message,
      loading: true,
  ...
  const renderDataAgent = () => {
    return (
      <div
        className={classNames("p-10 flex flex-col flex-1 w-0 max-w-[1200px]")}
      >
        <div className="w-full flex justify-between">
          <div className="w-full flex items-center pb-8">
            {/* <Logo /> */}
        ...
        <div className="w-full relative h-full mb-10" ref={chatRef}>
  ...
  return (
    <div className="h-full w-full flex justify-center">
      <SpinLoadingFunction ref={spinLoadingRef} />
...
export default ChatViewHistory;
